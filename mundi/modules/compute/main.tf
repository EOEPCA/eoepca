...
resource "opentelekomcloud_networking_secgroup_rule_v2" "bastion" {
  ...
  security_group_id = "${opentelekomcloud_networking_secgroup_v2.bastion[count.index].id}"
}
...
resource "null_resource" "provision-bastion" {
...
resource "opentelekomcloud_compute_instance_v2" "bastion_custom_volume_size" {
  name       = "${var.cluster_name}-bastion-${count.index + 1}"
  count      = "${var.bastion_root_volume_size_in_gb > 0 ? var.number_of_bastions : 0}"
  image_name = "${var.image}"
  flavor_id  = "${var.flavor_bastion}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_master" {
  ...
  security_groups = ["${opentelekomcloud_networking_secgroup_v2.k8s_master.name}",
...
resource "opentelekomcloud_compute_instance_v2" "k8s_master_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-master-${count.index + 1}"
  count             = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_master_no_etcd_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-master-ne-${count.index + 1}"
  count             = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters_no_etcd : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "etcd_custom_volume_size" {
  name              = "${var.cluster_name}-etcd-${count.index + 1}"
  count             = "${var.etcd_root_volume_size_in_gb > 0 ? var.number_of_etcd : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_master_no_floating_ip_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-master-nf-${count.index + 1}"
  count             = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters_no_floating_ip : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_master_no_floating_ip_no_etcd_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-master-ne-nf-${count.index + 1}"
  count             = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters_no_floating_ip_no_etcd : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_node_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-node-${count.index + 1}"
  count             = "${var.node_root_volume_size_in_gb > 0 ? var.number_of_k8s_nodes : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_instance_v2" "k8s_node_no_floating_ip_custom_volume_size" {
  name              = "${var.cluster_name}-k8s-node-nf-${count.index + 1}"
  count             = "${var.node_root_volume_size_in_gb > 0 ? var.number_of_k8s_nodes_no_floating_ip : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
...
resource "opentelekomcloud_compute_floatingip_associate_v2" "bastion_custom_volume_size" {
  count                 = "${var.bastion_root_volume_size_in_gb > 0 ? var.number_of_bastions : 0}"
  floating_ip           = "${var.bastion_fips[count.index]}"
  instance_id           = "${element(opentelekomcloud_compute_instance_v2.bastion_custom_volume_size.*.id, count.index)}"
  #wait_until_associated = "${var.wait_for_floatingip}"
}
...
resource "opentelekomcloud_compute_floatingip_associate_v2" "k8s_master_custom_volume_size" {
  count                 = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters : 0}"
  instance_id           = "${element(opentelekomcloud_compute_instance_v2.k8s_master_custom_volume_size.*.id, count.index)}"
  floating_ip           = "${var.k8s_master_fips[count.index]}"
  #wait_until_associated = "${var.wait_for_floatingip}"
}

resource "opentelekomcloud_compute_floatingip_associate_v2" "k8s_master_no_etcd" {
  count       = "${var.master_root_volume_size_in_gb == 0 ? var.number_of_k8s_masters_no_etcd : 0}"
  instance_id = "${element(opentelekomcloud_compute_instance_v2.k8s_master_no_etcd.*.id, count.index)}"
  floating_ip = "${var.k8s_master_no_etcd_fips[count.index]}"
}

resource "opentelekomcloud_compute_floatingip_associate_v2" "k8s_master_no_etcd_custom_volume_size" {
  count       = "${var.master_root_volume_size_in_gb > 0 ? var.number_of_k8s_masters_no_etcd : 0}"
  instance_id = "${element(opentelekomcloud_compute_instance_v2.k8s_master_no_etcd_custom_volume_size.*.id, count.index)}"
  floating_ip = "${var.k8s_master_no_etcd_fips[count.index]}"
}
...
resource "opentelekomcloud_compute_floatingip_associate_v2" "k8s_node_custom_volume_size" {
  count                 = "${var.node_root_volume_size_in_gb > 0 ? var.number_of_k8s_nodes : 0}"
  floating_ip           = "${var.k8s_node_fips[count.index]}"
  instance_id           = "${element(opentelekomcloud_compute_instance_v2.k8s_node_custom_volume_size.*.id, count.index)}"
  #wait_until_associated = "${var.wait_for_floatingip}"
}
...
resource "opentelekomcloud_blockstorage_volume_v2" "glusterfs_volume_custom_volume_size" {
  name        = "${var.cluster_name}-glusterfs_volume-${count.index + 1}"
  count       = "${var.gfs_root_volume_size_in_gb > 0 ? var.number_of_gfs_nodes_no_floating_ip : 0}"
  description = "Non-ephemeral volume for GlusterFS"
  size        = "${var.gfs_volume_size_in_gb}"
...
resource "opentelekomcloud_compute_instance_v2" "glusterfs_node_no_floating_ip_custom_volume_size" {
  name              = "${var.cluster_name}-gfs-node-nf-${count.index + 1}"
  count             = "${var.gfs_root_volume_size_in_gb > 0 ? var.number_of_gfs_nodes_no_floating_ip : 0}"
  #availability_zone = "${element(var.az_list, count.index)}"
  image_name        = "${var.image}"
  ...
  block_device {
    uuid                  = "${data.opentelekomcloud_images_image_v2.gfs_image.id}"
    source_type           = "image"
...
resource "opentelekomcloud_compute_volume_attach_v2" "glusterfs_volume_custom_root_volume_size" {
  count       = "${var.gfs_root_volume_size_in_gb > 0 ? var.number_of_gfs_nodes_no_floating_ip : 0}"
  instance_id = "${element(opentelekomcloud_compute_instance_v2.glusterfs_node_no_floating_ip_custom_volume_size.*.id, count.index)}"
  volume_id   = "${element(opentelekomcloud_blockstorage_volume_v2.glusterfs_volume_custom_volume_size.*.id, count.index)}"
}
