...
ADES User A retrieve status Job1
  ...
  ${a}=  API_PROC Check Job Status Success  ${ADES_BASE_URL}  ${LOCATION}  ${access_token}
  Status Should Be  200  ${a}

ADES User B retrieve status Job1
  ${ticket}=  API_PROC Check Job Status for Ticket  ${UM_BASE_URL}  ${LOCATION}  ${RPT_TOKEN}
  ${access_token}=  UMA Get Access Token Valid  ${WELL_KNOWN_PATH}  ${ticket}  ${UB_TK}  ${C_ID_UMA}  ${C_SECRET_UMA}
  ${status}=  API_PROC Check Job Status Success  ${ADES_BASE_URL}  ${LOCATION}  ${RPT_TOKEN}
  Status Should Be  401  ${status}

ADES User B attempt execute Proc1
  ${resp}=  API_PROC Execute Process  ${ADES_BASE_URL}  ${API_PROC_PATH_PREFIX}  eo_metadata_generation_1_0  ${CURDIR}${/}eo_metadata_generation_1_0_execute.json  ${UB_TK}
...
ADES User A execute Proc1
  ${resp}=  API_PROC Execute Process  ${ADES_BASE_URL}  ${API_PROC_PATH_PREFIX}  eo_metadata_generation_1_0  ${CURDIR}${/}eo_metadata_generation_1_0_execute.json  ${UA_TK}
  ...
  ${val}=  API_PROC Execute Process  ${ADES_BASE_URL}  ${API_PROC_PATH_PREFIX}  eo_metadata_generation_1_0  ${CURDIR}${/}eo_metadata_generation_1_0_execute.json  ${rptA}
...
ADES User B undeploy Proc1
  ${resp}=  API_PROC Undeploy Process  ${ADES_BASE_URL}  ${API_PROC_PATH_PREFIX}  eo_metadata_generation_1_0  ${CURDIR}${/}eo_metadata_generation_1_0_undeploy.json  ${UB_TK}
  ${ticket}=  UMA Get Ticket From Response  ${resp}
...
ADES User A deploys Proc1
  ${resp}=  API_PROC Deploy Process  ${ADES_BASE_URL}  ${API_PROC_PATH_PREFIX}  eo_metadata_generation_1_0  ${CURDIR}${/}eo_metadata_generation_1_0.json  ${UA_TK}
  ${ticket}=  UMA Get Ticket From Response  ${resp}
  ${rptA}=  UMA Get Access Token Valid  ${WELL_KNOWN_PATH}  ${ticket}  ${UA_TK}  ${C_ID_UMA}  ${C_SECRET_UMA}
...
API_PROC Request Processes
  ...
  Log  ${headers}
  ${resp}=  Get Request  pep  ${path_prefix}/processes  headers=${headers}
  Log  ${resp}
  [Return]  ${resp}
...
API_PROC Check Job Status for Ticket
  ...
  ${resp}=  Get Request  pep  /${loc}  headers=${headers}
  ${ticket}=  UMA Get Ticket From Response  ${resp}
  [return]  ${ticket}
...
API_PROC execute process
  ...
  ${headers}=  Create Dictionary  accept=application/json  Prefer=respond-async  Content-Type=application/json  authorization=Bearer ${token}
  ${file_data}=  Get Binary File  ${filename}
  ${resp}=  Post Request  pep  ${path_prefix}/processes/eo_metadata_generation_1_0/jobs  headers=${headers}  data=${file_data}
  [Return]  ${resp}
