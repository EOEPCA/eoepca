...
ADES Protection as Service
  ...
  #User D protects the root path of ADES (“/wps3”) with an “Authenticated” scope
  PEP Register ADES
...
ADES User B undeploy Proc1
  ${rptB}=  UMA_Library.UMA Flow Setup  ${UM_BASE_URL}  hola  /ades${API_PROC_PATH_PREFIX}/processes/eoepcaadesdeployprocess/jobs  ${WELL_KNOWN_PATH}  ${USERB}  ${PASSWORD_USERS}
...
ADES User A deploys Proc1
  ${rptA}=  UMA_Library.UMA Flow Setup  ${UM_BASE_URL}  hola  /ades${API_PROC_PATH_PREFIX}/processes/eoepcaadesdeployprocess/jobs  ${WELL_KNOWN_PATH}  ${USERA}  ${PASSWORD_USERS}
  Log to Console  ${rptA}
...
UMA Flow Setup Ades
  ...
  ${resp_ticket}=  UMA_Library.UMA Get Ticket Valid  ${base_url}  ${token}  ${RES_ID_A}
...
PEP Register ADES
  ...
  ${a}=  Run Process  sh  ${CURDIR}${/}id.sh  -t  ${endpoint}  -i  ${CLIENT_ID}  -p  ${CLIENT_SECRET}
  ${n}=  OperatingSystem.Get File  ${CURDIR}${/}1.txt
  ${b}=  Run Process  python3  ${CURDIR}${/}insertADES.py  ${ADES_BASE_URL}  ${PEP_RESOURCE_PORT} 
