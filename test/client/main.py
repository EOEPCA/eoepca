...
def main():
    ...
    demo.add_resource(ades_url, "/watchjob", user_id_token, "ADES Jobs", ["Authenticated"])
    ...
    print("\n### API Proc Execute Application ###")
    app_name = "vegetation_index_"
    access_token, job_location = demo.proc_execute_application(ades_proc_url, app_name, "data/app-execute-body.json", id_token=user_id_token, access_token=access_token)

    # API Proc Get Job Status
    # job_location = "http://ades.test.185.52.193.87.nip.io/watchjob/processes/vegetation_index_/jobs/dbf198a6-226b-11eb-88d0-eaf745f58ace"
    job_location_path = urlparse(job_location).path
    print("\n### API Proc Get Job Status ###")
    status = "running"
    while status == 'running':
        access_token, response = demo.proc_get_job_status(active_ades_url + ades_prefix, job_location_path, id_token=user_id_token, access_token=access_token)
    ...
    print("\n### API Proc Get Job Result ###")
    access_token, response = demo.proc_get_job_result(active_ades_url + ades_prefix, job_location_path, id_token=user_id_token, access_token=access_token)
